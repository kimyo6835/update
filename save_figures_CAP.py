...
save_path = '/home/gimmedatcudaspeedmmmh/Desktop/NeuronGuys/JIN/capsule_test/idontwanna/sample_stack/d_300_3mg_silver/savefigures/capsule_epoch5'
...
for index in wireindex:
    ...
    resized_image = wires_functions.striding_image(path_to_raw, index, imsize, kernel_size, conv_imsize)
    ...
    start_point_x = np.multiply(stride_size, np.arange(0, resized_image.shape[0])).reshape(-1, batch_size)
    start_point_y = np.multiply(stride_size, np.arange(0, resized_image.shape[1]))

    k = np.arange(0, start_point_x.shape[1] + 1)
    tile_x = start_point_x
